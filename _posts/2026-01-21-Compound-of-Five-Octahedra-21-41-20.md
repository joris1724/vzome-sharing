...
When panels are introduced, for esthetical reasons, an altenative becomes available avoiding these superfluous connectors. Two adjacent panels can be 3D printed as one single object, internally reinforced along the shared edge. One could also regard this as a 3D printed strut, without pins and with the adjacent panels attached to it as wings. This technique is particularly worth while if the two panels have the same colour, and if the edge cannot be realized with one standard Zometool strut. Both conditions are met here, resulting in a much more appealing model:

    ...
    translate([DX*Dx,DY*Dy,0]) rotate([0,HY[F],0]) rotate([0,0,HZ[F]]) translate([P[FACES[F]][0],P[FACES[F]][1],P[FACES[F]][2]]) scale(SC) {
        difference() {
            render() {
                ...
                color(RGB[G]) translate([0,0,-1.06666666666667]) rotate([0,0,45]) rotate([0,-54.7356103172453,0]) cylinder(IR/SC,4/SC,4/SC,$fn=32);
            ...
            render() {
                ...
                if (F==2)
                    color(RGB[G]) translate([0,0,-1.06666666666667]) rotate([0,0,45]) rotate([0,-54.7356103172453,0])  translate(EPSILON) cylinder(S42[1][0],S42[1][1],S42[1][1],$fn=32);
                else
                    color(RGB[G]) translate([0,0,-1.06666666666667]) rotate([0,0,45]) rotate([0,-54.7356103172453,0])  translate(EPSILON) cylinder(S42[0][0],S42[0][1],S42[0][1],$fn=32);
                if (F==3)
...
single(0,0,2,0,1,2,3,4,5,6);
single(0,1,2,0,1,4,3,2,5,6);
single(1,0,2,0,3,2,1,4,5,6);
single(1,1,2,0,3,4,1,2,5,6);
translate ([112,25,0]) rotate([0,0,180]) {
    single(2,0,2,1,0,2,3,4,5,6);
    single(2,1,2,1,0,4,3,2,5,6);
    single(3,0,2,0,6,2,3,4,5,1);
    single(3,1,2,0,6,4,3,2,5,1);
}